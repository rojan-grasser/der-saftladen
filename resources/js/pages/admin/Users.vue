...
const debouncedFetchUsers = debounce((value: string) => fetchUsers(1, value), 500);
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col gap-6 p-6">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <Select v-model="roleFilter">
          <SelectTrigger class="w-40"><SelectValue placeholder="Alle Rollen" /></SelectTrigger>
          <SelectContent>
        ...
        <Select v-model="statusFilter">
          <SelectTrigger class="w-40"><SelectValue placeholder="Alle Status" /></SelectTrigger>
          <SelectContent>
        ...
        <Button
          ...
          variant="ghost"
          @click="() => { search=''; roleFilter='all'; statusFilter='all'; fetchUsers(1); }"
        >
          Leeren
        </Button>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="user in props.users.data" :key="user.id">
              ...
              <TableCell>
                <Badge class="capitalize" variant="secondary">{{ roleLabels[user.role as UserRole] }}</Badge>
              </TableCell>
              <TableCell>
                <Badge :variant="statusVariants[user.status as UserStatus]" class="capitalize">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <Button aria-label="Bearbeiten" size="icon-sm" @click="editUser(user)">
                        <Pencil />
                      </Button>
                    </TooltipTrigger>
    ...
    <!-- Edit Modal -->
    <UserEdit v-if="editingUser" v-model:open="isEditOpen" :user="editingUser" />
  </AppLayout>
</template>
